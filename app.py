...
TARGETS = [
    "운동시설", "교육_학습시설", "키즈시설",
    "시니어시설", "공유오피스_회의", "라운지_휴게",
]

BIZ_FEATURES = [
    "카페수", "헬스장수", "학원수", "기타학원수", "독서실수", "사우나수",
]

DEMO_FEATURES = [
    "ratio_child", "ratio_elderly", "ratio_young",
    "ratio_middle", "ratio_1person", "ratio_2person", "ratio_3plus",
]
...
MODEL_PERFORMANCE = [
    {"시설": "키즈시설",        "보유율": 55.3, "Baseline보유F1": 0.712, "채택모델": "Random Forest",       "보유F1": 0.580, "MacroF1": 0.547, "BalAcc": 0.549, "판정": "재검토"},
...
def inject_css():
    st.markdown(
        ...
        .hero {
            padding:2.4rem 2.6rem; border-radius:24px;
            background:linear-gradient(135deg,#050B14 0%,#0F2358 45%,#1D4ED8 100%);
            color:#F8F4E3; margin-bottom:1.4rem;
            box-shadow:0 20px 48px rgba(0,0,0,.45);
            border:1px solid rgba(56,189,248,.18);
        }
        .hero h1 { color:#F8F4E3; font-size:2.3rem; line-height:1.25; margin:0 0 .75rem 0; }
        .hero p  { color:#BAE6FD; font-size:1.05rem; line-height:1.65; margin:0; }

        .section-title {
            font-size:1.18rem; font-weight:800; color:#F2C879;
            margin-top:1.8rem; margin-bottom:.75rem;
            border-left:4px solid #F2C879; padding-left:.6rem;
        }

        .soft-card {
            padding:1.2rem 1.3rem; border-radius:18px;
            background:#0E1B2E; border:1px solid rgba(242,200,121,.15);
            box-shadow:0 6px 20px rgba(0,0,0,.3);
            margin-bottom:1rem; min-height:135px;
        }
        .soft-card h3 { font-size:1.03rem; margin:.2rem 0 .45rem 0; color:#F8F4E3; }
        .soft-card p  { color:#94A3B8; line-height:1.55; font-size:.92rem; margin:0; }

        .note-box {
            padding:1rem 1.2rem; border-radius:16px;
            background:rgba(29,78,216,.12); border:1px solid rgba(56,189,248,.25);
            color:#93C5FD; line-height:1.6; margin-bottom:1rem; font-size:.95rem;
        }
        .info-box {
            padding:.9rem 1.2rem; border-radius:14px;
            background:rgba(234,179,8,.08); border:1px solid rgba(242,200,121,.3);
            color:#FDE68A; line-height:1.6; margin-bottom:1rem; font-size:.93rem;
        }

        .badge { display:inline-block; padding:.28rem .65rem; border-radius:999px; font-size:.76rem; font-weight:800; margin-bottom:.5rem; }
        .badge-green  { background:rgba(34,197,94,.18);  color:#86EFAC; }
        .badge-red    { background:rgba(239,68,68,.18);  color:#FCA5A5; }
        .badge-yellow { background:rgba(234,179,8,.18);  color:#FDE68A; }
        .badge-blue   { background:rgba(56,189,248,.18); color:#7DD3FC; }

        /* 결론 카드 */
        .conclusion-card {
            padding:1.4rem 1.5rem; border-radius:20px;
            background:#0E1B2E; border:1px solid rgba(255,255,255,.09);
            box-shadow:0 8px 28px rgba(0,0,0,.35);
            margin-bottom:1.1rem; min-height:210px;
        ...
        .conclusion-card .verdict-stamp { display:inline-block; font-size:1.55rem; font-weight:900; letter-spacing:-.5px; margin-bottom:.5rem; }
        .verdict-green  { color:#22C55E; }
        .verdict-red    { color:#EF4444; }
        .verdict-yellow { color:#F59E0B; }
        .conclusion-card .q-label { display:inline-block; font-size:.72rem; font-weight:800; padding:.2rem .55rem; border-radius:6px; margin-right:.4rem; vertical-align:middle; letter-spacing:.03em; }
        .q-label-green  { background:rgba(34,197,94,.15);  color:#86EFAC; }
        .q-label-red    { background:rgba(239,68,68,.15);  color:#FCA5A5; }
        .q-label-yellow { background:rgba(245,158,11,.15); color:#FDE68A; }
        .conclusion-card h3 { font-size:1.08rem; margin:.35rem 0 .55rem 0; color:#F8F4E3; line-height:1.4; }
        .conclusion-card .detail-row { font-size:.88rem; color:#94A3B8; line-height:1.6; margin-bottom:.25rem; }
...
def load_data():
    if not DATA_PATH.exists():
        st.error("최종 데이터 파일을 찾을 수 없습니다. `data/output/apt_final_1123.csv`를 업로드하세요.")
        st.stop()
...
def _centroid(geometry):
    """GeoJSON geometry에서 단순 평균 centroid 반환."""
    coords = []
...
def make_gu_choropleth(gu_summary, geo_data, value_col, title, unit="%", color_scheme="YlGnBu"):
    ...
    folium.GeoJson(
        ...
        style_function=lambda f: {
            "fillColor": colormap(f["properties"].get(value_col))
                if f["properties"].get(value_col) is not None else "#333",
            "color": "#555",
            "weight": 1,
            "fillOpacity": 0.78,
        },
        highlight_function=lambda f: {
            "fillColor": "#FFD700", "color": "#000", "weight": 2, "fillOpacity": 0.85
        },
    ...
    # ── 구 이름 레이블 ──────────────────────────────────────
    label_layer = folium.FeatureGroup(name="구 이름", show=True)
...
CONCLUSION_ROWS = [
    {
        ...
        "hypothesis": "1인가구비율 UP -> 공유오피스·회의시설 UP",
        "verdict": "기각", "color": "red",
    ...
    },
    {
        ...
        "hypothesis": "아동비율 UP -> 키즈시설 UP",
        "verdict": "지지", "color": "green",
    ...
    },
    {
        ...
        "hypothesis": "고령비율 UP -> 시니어시설 UP",
        "verdict": "기각 / 재해석", "color": "red",
        "card_class": "card-red", "verdict_class": "verdict-red", "q_class": "q-label-red",
    ...
    },
    {
        ...
        "hypothesis": "외부 상권 UP -> 단지 내 유사 시설 DOWN",
        "verdict": "부분 지지", "color": "yellow",
        ...
        for col, row in zip(cols, CONCLUSION_ROWS[i:i + 2]):
            with col:
                st.markdown(
                    ...
                    '<b style="color:#F8F4E3;margin-left:.3rem;">' + row["corr"] + "</b>"
                    "&nbsp;&nbsp;"
                    '<span style="color:#64748B;">모델:</span>'
                    '<b style="color:#F8F4E3;margin-left:.3rem;">' + row["shap_rank"] + "</b>"
                    "</div>"
                    '<div class="detail-row" style="margin-top:.4rem;">' + row["reason"] + "</div>"
                    "</div>",
                    unsafe_allow_html=True,
                )
...
if page == "🏠 홈":
    ...
    st.markdown(
        ...
        "<p>인구 구조 · 세대 구성 · 주변 상권 데이터를 결합해<br>"
        "서울시 신축 아파트 단지의 커뮤니티 시설 공급 패턴을 분류 모델과 SHAP으로 분석합니다.</p>"
        "</div>",
        unsafe_allow_html=True,
    )
    ...
    c1.metric("분석 단지", f"{len(df):,}개")
    c2.metric("시설 타겟", f"{len(available_targets)}개")
    c3.metric("자치구", f"{df['구'].nunique()}개" if "구" in df.columns else "-")
    c4.metric("모델 피처", "18개")

    st.markdown(
        '<div style="margin-top:2rem;margin-bottom:.6rem;">'
        '<span style="font-size:.75rem;font-weight:800;letter-spacing:.12em;'
        'color:#F2C879;text-transform:uppercase;">Research Conclusions</span>'
        '<div style="font-size:1.5rem;font-weight:900;color:#F8F4E3;margin-top:.2rem;line-height:1.25;">'
        "Q1 ~ Q4 &nbsp;"
        '<span style="color:#94A3B8;font-weight:400;font-size:1.1rem;">최종 가설 검증 결과</span>'
        "</div></div>",
        unsafe_allow_html=True,
    )
    ...
    with a:
        st.markdown(
            ...
            "<p>1,123개 단지 기준 시설별 보유율과 변수 분포 및 상관관계를 확인합니다.</p>"
            "</div>",
            unsafe_allow_html=True,
        )
    with b:
        st.markdown(
            ...
            "<p>LR/RF 모델 보유 클래스 F1, Macro F1, Balanced Accuracy와 시설별 SHAP Top 3를 정리합니다.</p>"
            "</div>",
            unsafe_allow_html=True,
        )
    with c:
        st.markdown(
            '<div class="soft-card">'
            '<span class="badge badge-blue">Folium</span>'
            "<h3>서울 25구 시설 보급률 지도</h3>"
            "<p>시설별 보급률과 Q4 상권 변수를 자치구 이름과 함께 Choropleth 지도로 탐색합니다.</p>"
            "</div>",
            unsafe_allow_html=True,
        )
...
# ============================================================
    ...
    with col1:
        ...
        feat_df = pd.DataFrame(
            [{"변수명": k, "설명": v} for k, v in FEATURE_LABELS.items() if k in available_demo_features]
        )
        st.dataframe(feat_df, use_container_width=True, hide_index=True)
    with col2:
        st.markdown('<div class="section-title">상권 피처 (6개, log 변환)</div>', unsafe_allow_html=True)
        st.dataframe(pd.DataFrame({"변수명": available_biz_features}), use_container_width=True, hide_index=True)
...
# ============================================================
    ...
    selected_target = st.selectbox(
        "시설 선택", available_targets, format_func=lambda x: TARGET_LABELS.get(x, x),
    )
...
# ============================================================
    ...
    if geo_data is None:
        st.error("`maps/seoul_gu_geo_enriched.json` 파일이 없습니다. `maps/` 폴더에 업로드하세요.")
        st.stop()
    ...
    st.markdown(
        ...
        "구 이름이 지도 위에 표시되며, 마우스 오버 시 상세 수치를 볼 수 있습니다."
        "</div>",
        unsafe_allow_html=True,
    )
...
# ============================================================
    ...
    design_data = {
        "항목": ["타겟", "피처 수", "교차검증", "비교 모델", "클래스 불균형", "주 평가 지표"],
        "내용": [
    ...
    st.markdown(
        '<div class="info-box">'
        "<b>키즈시설 해석 주의</b><br>"
        "키즈시설 보유율이 55.3%로 높아 Majority Baseline의 보유F1이 0.712로 높게 나타납니다. "
        "키즈시설은 <b>Macro F1 · Balanced Accuracy</b>를 함께 해석하세요."
        "</div>",
        unsafe_allow_html=True,
    )

    st.markdown('<div class="section-title">최종 모델 성능 요약</div>', unsafe_allow_html=True)
    st.dataframe(pd.DataFrame(MODEL_PERFORMANCE), use_container_width=True, hide_index=True)

    st.markdown('<div class="section-title">보유 클래스 F1 비교</div>', unsafe_allow_html=True)
    f1_series = pd.Series({row["시설"]: row["보유F1"] for row in MODEL_PERFORMANCE}).sort_values(ascending=False)
    ...
    st.markdown(
        ...
        "SHAP 평균 절댓값은 변수 영향력의 크기를 나타내며, 방향성은 단변량 상관계수로 보완합니다."
        "</div>",
        unsafe_allow_html=True,
    )

    st.markdown('<div class="section-title">Q4 방향 분석</div>', unsafe_allow_html=True)
    st.dataframe(pd.DataFrame(Q4_DIRECTION), use_container_width=True, hide_index=True)

    tab1, tab2 = st.tabs(["SHAP 변수 중요도 차트", "Q4 방향 분석 차트"])
    with tab1:
        p = CHART_DIR / "chart8_shap_importance.png"
        if p.exists():
            st.image(str(p), use_container_width=True)
        else:
            st.caption(f"파일 없음: {p}")
    with tab2:
        p = CHART_DIR / "chart9_q4_direction.png"
        if p.exists():
            st.image(str(p), use_container_width=True)
        else:
            st.caption(f"파일 없음: {p}")
...
# ============================================================
    ...
    chart_files = {
        "그림 1. 시설별 보유율":                    {"file": "chart1_facility_rate.png",          "desc": "6개 시설 유형별 보유율과 보유 단지 수"},
        "그림 2. 인구·세대 변수 분포":              {"file": "chart2_feature_dist.png",            "desc": "7개 인구·세대 변수 분포 히스토그램"},
        "그림 3. 피처×타겟 상관관계 히트맵":        {"file": "chart3_corr_heatmap.png",            "desc": "18개 피처와 6개 시설 타겟 상관관계"},
        "그림 4. 시설 보유 여부별 특성 차이":       {"file": "chart4_facility_comparison.png",     "desc": "보유 vs 미보유 단지의 인구·세대 평균 차이"},
        "그림 5. 자치구별 시설 보급률 히트맵":      {"file": "chart5_gu_heatmap.png",              "desc": "서울 25개 자치구별 시설 보급률 히트맵"},
        "그림 6. Q1~Q4 핵심 가설 예비검증":         {"file": "chart6_hypothesis_scatter.png",      "desc": "Q1~Q3 단변량 및 분위별 보유율 검토"},
        "그림 7. 시설별 SHAP 변수 중요도":          {"file": "chart8_shap_importance.png",         "desc": "시설별 최종 모델의 SHAP 평균 절댓값 Top 변수"},
        "그림 8. Q4 상권 방향 분석":                {"file": "chart9_q4_direction.png",            "desc": "외부 상권과 단지 내 시설 보유율의 방향성"},
        "그림 9. Q1~Q4 최종 결론표":                {"file": "chart10_hypothesis_final_conclusion.png", "desc": "Q1~Q4 최종 판정표"},
    }

    st.markdown(
        '<div class="note-box">'
        "최종 결론 카드는 <b>✅ 최종 결론</b> 페이지에서도 확인할 수 있습니다."
        "</div>",
        unsafe_allow_html=True,
    )
